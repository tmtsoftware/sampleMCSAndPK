@REM samplemcsdeploy launcher script
@REM
@REM Environment:
@REM JAVA_HOME - location of a JDK home dir (optional if java on path)
@REM CFG_OPTS  - JVM options (optional)
@REM Configuration:
@REM SAMPLEMCSDEPLOY_config.txt found in the SAMPLEMCSDEPLOY_HOME.
@setlocal enabledelayedexpansion

@echo off


if "%SAMPLEMCSDEPLOY_HOME%"=="" (
  set "APP_HOME=%~dp0\\.."

  rem Also set the old env name for backwards compatibility
  set "SAMPLEMCSDEPLOY_HOME=%~dp0\\.."
) else (
  set "APP_HOME=%SAMPLEMCSDEPLOY_HOME%"
)
...
set "CFG_FILE=%APP_HOME%\SAMPLEMCSDEPLOY_config.txt"
set CFG_OPTS=
...
if "%JAVAOK%"=="false" (
  ...
  echo and download a valid Java JDK and install before running samplemcsdeploy.
  echo.
...
"%_JAVACMD%" !_JAVA_OPTS! !SAMPLEMCSDEPLOY_OPTS! -cp "%APP_CLASSPATH%" %MAIN_CLASS% !_APP_ARGS!
